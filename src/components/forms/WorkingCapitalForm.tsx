...
import { WORKING_CAPITAL_FIELD_CONFIGS } from '@/types/workingCapital';
...
export const WorkingCapitalForm: React.FC<WorkingCapitalFormProps> = ({ projectId }) => {
  const {
    formData,
    originalData,
    isLoading,
    ...
    hasUnsavedChanges,
    validateForm
  } = useWorkingCapitalData({ projectId });

  // Group fields by category
  const groupedFields = WORKING_CAPITAL_FIELD_CONFIGS.reduce((acc, field) => {
    if (!acc[field.category]) {
      acc[field.category] = [];
    }
    acc[field.category].push(field);
    return acc;
  }, {} as Record<string, typeof WORKING_CAPITAL_FIELD_CONFIGS>);
  ...
  const getCategoryDisplayName = (category: string) => {
    const categoryMap: Record<string, string> = {
      ...
      percentages: 'Working Capital Percentages'
    };
      ...
      <Card>
        <CardContent className="p-6">
          <div className="flex items-center justify-center py-8">
            ...
            <span className="ml-2">Loading working capital data...</span>
          </div>
        </CardContent>
      </Card>
    );
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-xl">Working Capital Assumptions</CardTitle>
              <CardDescription>
                Define working capital ratios and cash cycle parameters
              </CardDescription>
            </div>
        ...
        {Object.entries(groupedFields).map(([category, fields]) => (
          <Card key={category}>
            <CardHeader>
              ...
              <CardDescription>
                {category === 'days' 
                  ? 'Working capital cycle parameters in days'
                  : 'Working capital ratios as percentages of revenue'
                }
              </CardDescription>
            </CardHeader>
            <CardContent>
