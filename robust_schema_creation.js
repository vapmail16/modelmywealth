...
class RobustSchemaCreator {
  ...
  async setupExtensions() {
    try {
      this.log('🔧 Setting up PostgreSQL extensions...');
      
      // Enable UUID extension
      await this.productionClient.query('CREATE EXTENSION IF NOT EXISTS "uuid-ossp";');
      this.log('  ✅ UUID extension enabled');
      
      // Enable other common extensions
      await this.productionClient.query('CREATE EXTENSION IF NOT EXISTS "pgcrypto";');
      this.log('  ✅ PGCrypto extension enabled');
      
      return true;
    } catch (error) {
      this.log(`  ❌ Error setting up extensions: ${error.message}`, 'ERROR');
      return false;
    }
  }
    ...
    try {
      ...
      const columnDefs = columns.map(col => {
        ...
        if (col.column_default && col.column_default !== 'NULL') {
          // Replace uuid_generate_v4() with uuid_generate_v4()
          let defaultValue = col.column_default;
          if (defaultValue.includes('uuid_generate_v4()')) {
            defaultValue = defaultValue.replace('uuid_generate_v4()', 'uuid_generate_v4()');
          }
  ...
  async runSchemaCreation() {
    try {
      this.log('🚀 Starting robust production database schema creation...');
      ...
      if (!(await this.setupExtensions())) {
        throw new Error('Failed to setup required extensions');
      }
      ...
      const logPath = path.join(__dirname, `robust_schema_creation_log_${new Date().toISOString().replace(/[:.]/g, '-')}.txt`);
...
// Run schema creation
async function main() {
  const schemaCreator = new RobustSchemaCreator();
