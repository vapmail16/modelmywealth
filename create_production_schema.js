...
class SchemaCreator {
  ...
  async getTableConstraints(client, tableName) {
    const query = `
      SELECT 
        constraint_name,
        constraint_type,
  ...
  async getTableIndexes(client, tableName) {
    const query = `
      SELECT 
        indexname,
        indexdef
  ...
  async generateCreateTableSQL(tableName) {
    try {
      ...
      const constraints = await this.getTableConstraints(this.localClient, tableName);
      const indexes = await this.getTableIndexes(this.localClient, tableName);
      ...
      this.log(`Error generating SQL for table ${tableName}: ${error.message}`, 'ERROR');
      return null;
    }
  }
  ...
  async createTableInProduction(tableName) {
    try {
      ...
      const createSQL = await this.generateCreateTableSQL(tableName);
      if (!createSQL) {
        return false;
      }
  ...
  async runSchemaCreation() {
    try {
      ...
      const logPath = path.join(__dirname, `schema_creation_log_${new Date().toISOString().replace(/[:.]/g, '-')}.txt`);
